...
const { formatSideDirection, formatDateByShortPattern, formatDateByLongPattern } = require('./src/utils/commons')
...
async function updateWeatherInformation() {
  await fetch(
    ...
    .then(r => {
      ...
      DATA.wind_direction = formatSideDirection(r.wind.deg)

      DATA.weather = r.weather[0].description
      DATA.weather_icon = r.weather[0].icon

      DATA.sunrise = formatDateByShortPattern(r.sys.sunrise)
      DATA.sunset = formatDateByShortPattern(r.sys.sunset)
    })
}
