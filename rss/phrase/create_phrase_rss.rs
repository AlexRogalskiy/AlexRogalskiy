...
pub fn create_phrase_rss() -> std::io::Result<()> {
    let tpl =
        fs::read_to_string("README.md")
        .expect("Something went wrong reading the README.md file");

    let re = Regex::new(r"<!--views:phrase-rss-marker:start-->[\s\S]*?<!--views:phrase-rss-marker:end-->").unwrap();
    let last_articles = format!("{}{}\n{}", "<!--views:phrase-rss-marker:start-->", get_latest_articles(), "<!--views:phrase-rss-marker:end-->");
