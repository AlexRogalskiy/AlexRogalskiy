...
pub fn create_saying_rss() -> std::io::Result<()> {
    let tpl =
        fs::read_to_string("README.md")
        .expect("Something went wrong reading the README.md file");

    let re = Regex::new(r"<!--views:saying-rss-marker:start-->[\s\S]*?<!--views:saying-rss-marker:end-->").unwrap();
    let last_articles = format!("{}{}\n{}", "<!--views:saying-rss-marker:start-->", get_latest_articles(), "<!--views:saying-rss-marker:end-->");
...
fn get_blog_rss() -> Vec<FeedItem> {
    return Channel::from_url("https://www.englishclub.com/ref/saying-of-the-day.xml")
        .unwrap()
        .items()
        .iter()
        .map(|item| FeedItem {
