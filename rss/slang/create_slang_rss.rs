...
pub fn create_slang_rss() -> std::io::Result<()> {
    ...
    let re = Regex::new(r"<!--views:slang-rss-marker:start-->[\s\S]*?<!--views:slang-rss-marker:end-->").unwrap();
    let last_articles = format!("{}{}\n{}", "<!--views:slang-rss-marker:start-->", get_latest_articles(), "<!--views:slang-rss-marker:end-->");
...
fn get_blog_rss() -> Vec<FeedItem> {
    return Channel::from_url("https://www.englishclub.com/ref/slang-of-the-day.xml")
        .unwrap()
        .items()
        .iter()
        .map(|item| FeedItem {
